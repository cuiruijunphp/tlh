...
class UserSkillModel extends CommonModel{
	...
	public function get_skill_list($where = null, $page = 1, $page_size = 10, $order = 'add_time desc'){

		return $this->field('s.id,s.skill_name,s.type_id,s.desc,s.price,s.superiority,s.mode,s.img,s.status,s.good_at,s.user_id,u.user_name,t.type_name,s.add_time')
			->alias('s')
			->where($where)
