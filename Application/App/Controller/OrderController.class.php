...
//				$this->result_return(null, 1, '创建订单失败,金额非法');
//			}
			...
			if($source_type == 2){
				...
				if(!$demand_info){
					$this->result_return(null, 1, '该条需求不满足付款条件');
				}
				...
				if(!$reserve_info){
					$this->result_return(null, 1, '不存在的预约,不能付款哦');
				}
			}
			...
			if(!in_array($price, $earnest_money_list)){
				$this->result_return(null, 1, '诚意金不在可选范围');
			}
		...
		if($insert_result === false){
			$this->result_return(null, 1, '创建订单失败');
		}
